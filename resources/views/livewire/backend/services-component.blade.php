        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden shadow-xl rounded p-4">
                    <h2 class="font-extrabold text-teal-900 hover:text-teal-600 ml-3 text-2xl">My Services</h2>
                    <div class="row">
                            ...
                            <tbody>
                                @forelse ($services as $service )
                                    <tr>
                                       <td >{{$service->id}}</td>
                                       <td>{{$service->title}}</td>
                                       <td>{{\Carbon\Carbon::parse($service->created_at)->isoFormat('MMM Do YYY')}}</td>
                                      
   
                                       <td>
                                        <a href="{{route('service_edit', ['id'=>$service->id])}}" class="btn btn-primary text-white">Edit</a>
                                        <a href="#" class="btn btn-danger rounded  text-white" wire:click.prevent="deleteService('{{$service->id}}')">Delete</a>
                                       </td>
                                    </tr>                                
                                @empty
